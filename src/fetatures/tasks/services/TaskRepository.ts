...
class TaskRepository implements ITaskRepository {
  ...
  async findById(taskId: number) {
    const [result] = await db.select().from(task).where(eq(task.id, taskId)).limit(1);
    return result;
  }
  ...
  async update(taskId: number, data: Partial<SelectTask>) {
    const [result] = await db.update(task).set({ ...data }).where(eq(task.id, taskId)).returning();
    return result;
  }
  ...
  async listCommunityOptions() {
    return db.select({ id: community.id, name: community.name }).from(community).orderBy(asc(community.name));
  }
  ...
  async listNplOptions() {
    return db.select({ id: npl.id, tituloOperacion: npl.tituloOperacion }).from(npl).orderBy(asc(npl.tituloOperacion));
  }
}
